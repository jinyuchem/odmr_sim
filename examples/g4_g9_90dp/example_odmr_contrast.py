#!/usr/bin/env python3
"""
ODMR contrast simulation for G4-G9 NV configuration at 90 degrees.

This example demonstrates ODMR simulations using g4_g9_90dp preset.
...
def main():
    ...
    ax.set_title("G4-G9 90dp ODMR Contrast")
    plt.savefig("g4_g9_90dp_contrast_vs_gamma.pdf", bbox_inches='tight', dpi=300)

    # --- Part 2: ODMR Spectrum with TWO peaks ---
    print("=== ODMR Spectrum ===")

    # For NV configuration: + and - are at DIFFERENT frequencies (non-degenerate)
    # From plot_v3.py: center = 2448.65 MHz, splitting = 808.28 MHz
    peak_freq_minus = (2448.65 - 808.28) * 1e-3  # 1.640 GHz (- transition)
    peak_freq_plus = (2448.65 + 808.28) * 1e-3   # 3.257 GHz (+ transition)
    ...
    frequencies = np.linspace(1, 4, 1001)  # GHz - wider range for split peaks
    linewidth = 0.02  # GHz
    ...
    plot_odmr_spectrum(
        ...
        ax=ax,
        xlim=(1, 4),
        title="G4-G9 90dp ODMR Spectrum (Split Peaks)"
    )

    plt.tight_layout()
    plt.savefig("g4_g9_90dp_odmr_spectrum.pdf", bbox_inches='tight', dpi=300)
    plt.show()
