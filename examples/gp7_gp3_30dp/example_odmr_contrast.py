#!/usr/bin/env python3
"""
ODMR contrast simulation for GP7-GP3 NV configuration at 30 degrees.

This example demonstrates ODMR simulations using gp7_gp3_30dp preset.
...
def main():
    # Load GP7-GP3 30dp preset (split +/- levels)
    model = get_preset('gp7_gp3_30dp')
    ...
    ax.set_title("GP7-GP3 30dp ODMR Contrast")
    plt.savefig("gp7_gp3_30dp_contrast_vs_gamma.pdf", bbox_inches='tight', dpi=300)

    # --- Part 2: ODMR Spectrum with TWO peaks ---
    print("=== ODMR Spectrum ===")

    # For NV configuration: + and - are at DIFFERENT frequencies (non-degenerate)
    # From plot_v3.py: center = 3340.17 MHz, splitting = 370.32 MHz
    peak_freq_minus = (3340.17 - 370.32) * 1e-3  # 2.970 GHz (- transition)
    peak_freq_plus = (3340.17 + 370.32) * 1e-3   # 3.710 GHz (+ transition)
    ...
    frequencies = np.linspace(1, 4.2, 1001)  # GHz
    linewidth = 0.02  # GHz
    ...
    plot_odmr_spectrum(
        ...
        ax=ax,
        xlim=(1, 4.2),
        title="GP7-GP3 30dp ODMR Spectrum (Split Peaks)"
    )

    plt.tight_layout()
    plt.savefig("gp7_gp3_30dp_odmr_spectrum.pdf", bbox_inches='tight', dpi=300)
    plt.show()
